...
export const COMPONENTS = {
  ...
  icon: {
    ...
    group: "fundamentos",
    detallado: true,
    description: "Un ícono de lucide con los tamaños, los tonos y la semántica del sistema: decoración por defecto, imagen con nombre si tiene `label`.",
    keyboard: [["—", "No es interactivo. Si se puede apretar, va adentro de un `Button`."]],
    a11y: [
    ...
    props: {
      Icon: {
        icon: "El componente de lucide: `icon={SearchIcon}`. Va como referencia, no como elemento.",
        size: "`sm` 16px · `md` 20px · `lg` 24px.",
        tone: "`current` (hereda) · `muted` · `subtle` · `brand` · `success` · `warning` · `danger`.",
        label: "Nombre accesible. Con él, `role=\"img\"`; sin él, `aria-hidden`.",
        className: PROP_DESCRIPTIONS.className,
      },
    },
    related: ["button", "badge", "empty-state"],
  },
  badge: {
    title: "Badge",
    group: "fundamentos",
  ...
  chart: {
    ...
    group: "contenido",
    detallado: true,
    description: "Gráficos sobre Recharts con los tokens del sistema: paleta de cinco series validada para daltonismo, tooltip y leyenda propios, y animación que respeta `prefers-reduced-motion`.",
    keyboard: [
    ...
    props: {
      ChartContainer: {
        config: "Las series, por `dataKey`: etiqueta, color opcional e ícono opcional para la leyenda. El orden de las claves es el orden de la paleta.",
        className: PROP_DESCRIPTIONS.className,
        style: "Se fusiona con las variables `--color-<serie>` que pone el contenedor.",
      },
      ChartTooltipContent: {
        indicator: "`dot` (un punto) · `line` (una barra vertical) · `none`.",
        hideLabel: "Oculta el título del tooltip (el valor del eje X).",
        hideIndicator: "Oculta el testigo de color.",
        labelKey: "Clave del payload que da el título. Por defecto, el `label` del eje.",
        nameKey: "Clave del payload que identifica la serie, para un `Pie` donde cada sector es una fila.",
        formatter: "Cómo se escribe cada valor: `(value) => \`$ ${value}\``.",
        labelFormatter: "Cómo se escribe el título.",
        active: "Lo pone Recharts: si el puntero está sobre un punto.",
        payload: "Lo pone Recharts: las series bajo el puntero.",
        label: "Lo pone Recharts: el valor del eje X bajo el puntero.",
        className: PROP_DESCRIPTIONS.className,
      },
      ChartLegendContent: {
        nameKey: "Igual que en el tooltip: la clave que identifica la serie.",
        hideIcon: "Ignora el `icon` de la config y muestra el punto de color.",
        verticalAlign: "Lo pone Recharts desde `<Legend verticalAlign>`: solo cambia de qué lado va el margen.",
        payload: "Lo pone Recharts.",
        className: PROP_DESCRIPTIONS.className,
      },
    },
    related: ["stat", "card", "table"],
  },
}
